...
export const cookieOptions: CookieOptions = { httpOnly: true, secure: env.NODE_ENV === "production", sameSite: "lax", path: "/" };
export const sessionSeconds = 60 * 60 * 24 * 7;

export const authenticate: RequestHandler = async (req, res, next) => {
  const token = req.cookies[cookieName];
  if (typeof token !== "string") throw new AppError(401, "UNAUTHENTICATED", "Sign in to continue.");
  let userId: string;
  try {
    const payload = jwt.verify(token, env.JWT_SECRET, { algorithms: ["HS256"], issuer: "taskora", audience: "taskora-web" });
